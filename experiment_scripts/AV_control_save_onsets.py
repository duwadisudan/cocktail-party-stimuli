"""Audio-visual block runner with extended pre-stimulus hold and trigger logging.

A variant of the AV runner that adds a jittered pre-stimulus crosshair hold
on the target screen (15-17 s) before each AV trial. Serial triggers are
issued at the cue/onset and trigger times are written to CSV alongside trial
responses.
...
filename_trigger_onsets = f"trigger_onsets_{current_datetime}.csv"
...
class WholeExperiment:
    ...
    def __init__(self):
        ...
        self.trigger_csv_path = f"C:\Sudan\Stimuli\initialization_data\\{filename_trigger_onsets}"
    ...
    def play_white_noise_and_crosshair(self, win, target_screen, hold_jitter_range=None):
        # Determine spatialized white-noise file for the target side
        ...
        crosshair = visual.ShapeStim(
            win,
            vertices=((-0.2, 0), (0.2, 0), (0, 0), (0, -0.1), (0, 0.1)),
            lineWidth=20,
            closeShape=False,
            lineColor='white'
        )

        # Send trigger and present 2s cue on target screen only
        ...
        # Added: sustain crosshair on target screen for jittered 15–17s (for AV trials)
        if hold_jitter_range is not None:
            hold_duration = random.uniform(hold_jitter_range[0], hold_jitter_range[1])
            # Keep the last frame on screen; do not flip other windows (center remains blank)
            core.wait(hold_duration)
        # Note: Do NOT flip/clear the center screen here
    ...
    def run_audio_visual_trial(self, win1, win2, win3, target_movie, masker_movie, target_screen, masker_screen, center_screen):
        ...
        # Ensure center screen is blank before cue/hold
        self.win3.clearBuffer()
        self.win3.flip()

        # Cue + added pre-stimulus hold on target screen (15–17s jitter)
        self.play_white_noise_and_crosshair(win1, target_screen, hold_jitter_range=(15, 17))

        # New trigger: right before starting movie playback
        self.send_neurospec_trigger("movie_start")
    ...
    def display_block_start(self, win, block_name):
        ...
        announcement = visual.TextStim(win, text=f"Starting {block_name}. Press spacebar to continue.  Starts after 10 seconds of crosshair at the center.", pos=(0, 0),height=0.05)
        announcement.draw()
        win.flip()
        event.waitKeys(keyList=["space"])
    ...
    def run_full_experiment(self):
        ...
        self.block_methods = ['run_audio_visual_trial']
        ...
        for block_method_name in self.block_methods:
            ...
            for target_movie, masker_movie in next_block_movie_pairs:
                ...
                if trial_count in (20, 40):  # Check if it's time for a break
    ...
    def save_trigger_timings(self):
        ...
        if not self.trigger_timings:
            return  # No timing data to save
            
        try:
            with open(self.trigger_csv_path, 'w', newline='') as csv_file:
                fieldnames = [
                    'trigger_time_from_start', 'absolute_timestamp', 'trial_number', 
                    'event_type', 'block_type', 'target_movie', 'masker_movie', 'system_datetime'
                ]
                csv_writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
                csv_writer.writeheader()
                csv_writer.writerows(self.trigger_timings)
            print(f"Trigger timing data saved to: {self.trigger_csv_path}")
        except Exception as e:
            print(f"Warning: Could not save trigger timings: {e}")
