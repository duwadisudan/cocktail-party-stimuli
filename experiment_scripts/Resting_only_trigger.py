"""Resting-state block runner with NeuroSpec trigger logging.

Runs the resting-state block. The participant fixates on a central
crosshair while serial triggers are issued at fixed intervals; trigger
times are written to CSV.

Outputs (under initialization_data/):
    response_<TIMESTAMP>.csv         per-trial metadata
    iniList_<TIMESTAMP>.csv          ordered list for this run
...
class WholeExperiment:
    ...
    def run_audio_only_trail(self, win1, win2, win3, target_movie, masker_movie, target_screen, masker_screen, center_screen):
        ...
        resting_duration = 300

        ## this one is modified from audio only for resting run
    ...
    def display_block_start(self, win, block_name):
        ...
        announcement = visual.TextStim(win, text=f"Starting Resting run. Press spacebar to continue. Starts after 10 seconds of crosshair at the center.", pos=(0, 0),height=0.05)
        announcement.draw()
        win.flip()
        event.waitKeys(keyList=["space"])
    ...
    def run_full_experiment(self):
        ...
        for block_method_name in self.block_methods:
            ...
            for target_movie, masker_movie in next_block_movie_pairs:
                ...
                if trial_count == 1:  # Check if it's time for a break
