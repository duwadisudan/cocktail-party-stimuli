"""Audio-only block runner with NeuroSpec trigger logging.

Plays target/masker pairs of spatialized speech as audio only (videos are
loaded but rendered at 1x1 px so they remain hidden). A central crosshair
is shown on the front-facing screen for fixation. Serial triggers are issued
at trial start and trigger times are written to CSV alongside trial
responses.

Outputs (under initialization_data/):
    response_<TIMESTAMP>.csv         per-trial responses
    iniList_<TIMESTAMP>.csv          target/masker pair list for this run
    trigger_onsets_<TIMESTAMP>.csv   high-precision trigger times

Hardware: three displays (screens 0/1/2), serial trigger box on COM3,
spatialized stereo audio.

Authorship note: AI tools assisted with documentation, commenting, and
refactoring of this script. The author is solely responsible for the
scientific correctness of the code and the validity of any analyses
produced with it.
"""
...
class WholeExperiment:
    ...
    def __init__(self):
        ...
        self.trial_count = 0
        self.csv_path = f"C:\\Users\\sudan\\Stimuli\\initialization_data\\{filename_iniList}"
        self.spatial_video_folder = r"C:\Users\sudan\Stimuli\sudan_spatial_videos_updated"
        self.response_csv_path = f"C:\\Users\\sudan\\Stimuli\\initialization_data\\{filename_response}"
        
        self.filename_trigger_onsets = f"trigger_onsets_{current_datetime}.csv"
        self.trigger_timing_path = f"C:\\Users\\sudan\\Stimuli\\initialization_data\\{self.filename_trigger_onsets}"
        self.trigger_timings = []
        self.experiment_start_time = time.perf_counter()
        self.current_trial_context = {}
    ...
    your_target_directory_path = 'C:\\Users\\sudan\\Stimuli\\Screenshots_faces_full_updated'
    your_save_directory_path = 'C:\\Users\\sudan\\Stimuli\\initialization_data'
    ...
    def send_neurospec_trigger(self, event_type="trial_start"):
        trigger_time = time.perf_counter()
        time_from_start = trigger_time - self.experiment_start_time
        
        self.ser.write(str.encode('0'))
        time.sleep(0.3)
        self.ser.write(str.encode('1'))
        
        self.trigger_timings.append({
            'trigger_time_from_start': time_from_start,
            'absolute_timestamp': trigger_time,
            'trial_number': self.trial_count,
            'event_type': event_type,
            'block_type': getattr(self, 'current_block_type', 'unknown'),
            'target_movie': getattr(self, 'current_target_movie', 'unknown'),
            'masker_movie': getattr(self, 'current_masker_movie', 'unknown'),
            'system_datetime': datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")
        })

    def save_trigger_timings(self):
        """Write the in-memory trigger log to CSV."""
        if not self.trigger_timings:
            return  # No data to save
        
        try:
            # Create CSV with header if file doesn't exist
            file_exists = os.path.exists(self.trigger_timing_path)
            
            with open(self.trigger_timing_path, 'a', newline='') as csv_file:
                fieldnames = ['trigger_time_from_start', 'absolute_timestamp', 'trial_number', 
                             'event_type', 'block_type', 'target_movie', 'masker_movie', 'system_datetime']
                ...
                writer.writerows(self.trigger_timings)
            
            self.trigger_timings = []
            
        except Exception as e:
            print(f"Warning: Failed to save trigger timings: {e}")

    def log_trial_context(self, target_movie, masker_movie, block_type):
        """Record current target/masker/block for the next trigger event."""
        self.current_target_movie = target_movie
        self.current_masker_movie = masker_movie
    ...
    def load_transcripts(self):
        transcripts_dict = {}
        with open(r"C:\\Users\\sudan\\Stimuli\\Transcripts\\english_translations_part2_copy.csv", 'r', encoding='ISO-8859-1') as csv_file:
            csv_reader = csv.DictReader(csv_file)
            for row in csv_reader:
    ...
    def load_screenshots(self):
        screenshot_folder = r"C:\\Users\\sudan\\Stimuli\\Screenshots_faces_full_updated"
    ...
    def play_white_noise_and_crosshair(self, win, target_screen):
        ...
        self.send_neurospec_trigger("white_noise_crosshair_start")
        white_noise.play()
        crosshair.draw()
    ...
    def prepare_screenshot_stimuli(self, win1, target_movie, masker_movie):
        ...
        img_all = [visual.ImageStim(win1, image=f"C:\\Users\\sudan\\Stimuli\\Screenshots_faces_full_updated\\{file}.jpg", size=(0.4, 0.2)) for file in all_screenshots]
    ...
    def run_audio_visual_trial(self, win1, win2, win3, target_movie, masker_movie, target_screen, masker_screen, center_screen):
        ...
        self.log_trial_context(target_movie, masker_movie, "audio_visual")

        inter_trial_interval  = random.uniform(10, 12)
        ...
        target_spatial_suffix = "_30" if target_screen == 1 else "_-30"
        masker_spatial_suffix = "_30" if masker_screen == 1 else "_-30"        # Store the responses
    ...
    def run_audio_only_trail(self, win1, win2, win3, target_movie, masker_movie, target_screen, masker_screen, center_screen):
        ...
        self.log_trial_context(target_movie, masker_movie, "audio_only")
        ...
        if self.trial_count == 0 or self.trial_count == 30:
            core.wait(10)

        self.send_neurospec_trigger("audio_only_trial_start")
    ...
    def run_control_trial(self, win1, win2, target_screen, masker_screen):
        ...
        with open('C:\\Users\\sudan\\Stimuli\\eye_no_sound.csv', 'a', newline='') as file:
    ...
    def display_block_start(self, win, block_name):
        ...
        announcement = visual.TextStim(win, text=f"Starting {block_name}. Press spacebar to continue.  Starts in 10 seconds.", pos=(0, 0),height=0.05)
        announcement.draw()
        win.flip()
        event.waitKeys(keyList=["space"])
    ...
    def run_full_experiment(self):
        ...
        self.block_methods = ['run_audio_only_trail']
        ...
        for block_method_name in self.block_methods:
            ...
            for target_movie, masker_movie in next_block_movie_pairs:
                ...
                if trial_count == 30:  # Check if it's time for a break
    ...
    def end_message(self):
        self.save_trigger_timings()
        
        win3 = visual.Window(size=(1080, 1920), screen=2, fullscr=False)
        closing_message = visual.TextStim(win3, 
                                        text="Please stay still until the experimenter says it is okay to move. Experimenter will change Tobii Battery.", 
                                        pos=(0, 0), height=0.05)
    ...
    def break_message(self):
        ...
        closing_message.draw()
        win3.flip()
        event.waitKeys(keyList=["space"])  # Wait for a spacebar press to close        self.win1.clearBuffer()
        self.win2.clearBuffer()
        self.win3.clearBuffer()
