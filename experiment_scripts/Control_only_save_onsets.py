"""Control block runner with NeuroSpec trigger logging.

Runs the eye-movement control block: on each trial, the participant is
instructed to move their eyes left or right. Serial triggers are issued at
each instruction onset and trigger times are written to CSV alongside trial
metadata.

Outputs (under initialization_data/):
    response_<TIMESTAMP>.csv         per-trial responses
    iniList_<TIMESTAMP>.csv          shuffled trial list for this run
    trigger_timings_<TIMESTAMP>.csv  high-precision trigger times
...
class WholeExperiment:
    ...
    def __init__(self):
        ...
        self.trigger_timings = []  # In-memory storage for trigger events
        self.experiment_start_time = time.perf_counter()
        self.trigger_csv_path = f"C:\Sudan\Stimuli\initialization_data\\trigger_timings_{current_datetime}.csv"
        self.current_target_movie = ""
        self.current_masker_movie = ""
    ...
    def send_neurospec_trigger(self, event_type="trial_start"):
        """Send a serial trigger and append timing metadata to the in-memory log."""
        trigger_time = time.perf_counter()
        
        # Send actual trigger to hardware
        self.ser.write(str.encode('0'))
        time.sleep(0.3)
        self.ser.write(str.encode('1'))
        
        # Log timing data to memory (no file I/O during trials)
        timing_data = {
            'trigger_time_from_start': trigger_time - self.experiment_start_time,
            'absolute_timestamp': trigger_time,
            'trial_number': self.trial_count,
            'event_type': event_type,
            'block_type': self.current_block_type,
            'target_movie': self.current_target_movie,
            'masker_movie': self.current_masker_movie,
            'system_datetime': datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")        }
        self.trigger_timings.append(timing_data)
    ...
    def save_trigger_timings(self):
        ...
        try:
            ...
            file_exists = os.path.exists(self.trigger_csv_path)
            
            with open(self.trigger_csv_path, 'a', newline='') as csv_file:
    ...
    def run_audio_visual_trial(self, win1, win2, win3, target_movie, masker_movie, target_screen, masker_screen, center_screen):
        ...
        # Store the responses        self.record_trial_data(target_movie, masker_movie, user_response, correct, transcript_response, transcript_correct, target_spatial_suffix, masker_spatial_suffix, "audio_visual")
    ...
    def run_audio_only_trail(self, win1, win2, win3, target_movie, masker_movie, target_screen, masker_screen, center_screen):
        ...
        # Store the responses        self.record_trial_data(target_movie, masker_movie, user_response, correct, transcript_response, transcript_correct, target_spatial_suffix, masker_spatial_suffix, "audio_only")
    ...
    def run_control_trial(self, win1, win2, target_screen, masker_screen):
        ...
        self.send_neurospec_trigger("control_trial_start")
        announcement.draw()
        self.win3.flip()
        core.wait(1)
        self.win3.flip()

        core.wait(4) #from the moment of command to another command it will be 5 seconds
    ...
    def run_control_block(self):
        ...
        announcement = visual.TextStim(self.win3, text=f"Starting control block. Press spacebar to continue. Starts after 10 seconds of crosshair at the center.", pos=(0, 0),height=0.05)
        announcement.draw()
    ...
    def break_message(self):
        ...
        win3 = visual.Window(size=(1080, 1920), screen=2, fullscr=False)
        closing_message = visual.TextStim(win3, 
                                        text="Please stay still until the experimenter says it is okay to move. Take a break and press spacebar to continue once you are ready. Experimenter will change Tobii Battery.", 
                                        pos=(0, 0), height=0.05)
        closing_message.draw()
        win3.flip()
