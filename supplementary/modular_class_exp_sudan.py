...
class AudioVisualExperiment:
    ...
    def __init__(self):
        ...
        self.info = StreamInfo(name='Your_Experiment_Name', type='Markers', channel_count=1, channel_format='int32', source_id='unique_id_001')
    ...
    def send_neurospec_trigger(self,trigger_code):
        self.ser.write(bytes([trigger_code]))

    def send_lsl_trigger(self, trigger_code):
        """Send a trigger code via LSL."""
        self.outlet.push_sample([trigger_code])
    ...
    def play_white_noise_and_crosshair(self, win, target_screen):
        ...
        circle = visual.ShapeStim(win, radius=0.05, edges=32, fillColor='white', lineColor='white',pos=(0, -1920 / 2 + 0.05))

        circle.draw()

        
        #self.send_neurospec_trigger(1)
        self.send_lsl_trigger(1)
        white_noise.play()
        crosshair.draw()
        win.flip()
        core.wait(2)
    ...
    def prepare_screenshot_stimuli(self, win1, target_movie):
        ...
        img_all = [visual.ImageStim(win1, image=f"C:\\Users\\mn0mn\\Documents\\Stimuli\\Sudan_face\\Screenshots_faces\\{file}.jpg", size=(0.4, 0.2)) for file in all_screenshots]
