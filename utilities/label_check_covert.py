"""Stimulus-label / covert-attention check harness.

Standalone harness used to verify that target/masker labels and screen
assignments line up with the audio-visual stimulus before running a real
session.
...
class AudioVisualExperiment:
    ...
    def load_and_play_movies_no_video(self, win1, win2, movie1_path, movie2_path, play_audio = True):
        ...
        mov1 = visual.MovieStim3(win1, movie1_path, size=hidden_video_size, noAudio=not play_audio)
        mov2 = visual.MovieStim3(win2, movie2_path, size=hidden_video_size, noAudio=not play_audio)
    ...
    def prepare_screenshot_stimuli(self, win1, target_movie):
        ...
        random_screenshots = random.sample([file for file in self.list_of_all_screenshots if file != target_movie], 2)
    ...
    def prepare_transcript_stimuli(self, win3, target_movie):
        ...
        random_transcripts = random.sample([trans for key, trans in self.transcripts_dict.items() if key != target_movie], 2)
    ...
    def select_screens(self):
        ...
        target_screen = random.choice([0, 0])
    ...
    def run_audio_only_trail(self, win1, win2, win3, target_movie, masker_movie, target_screen, masker_screen, center_screen):
        ...
        self.load_and_play_movies_no_video(win1, win2, movie1_path, movie2_path)
    ...
    def run_control_trial(self, win1, win2, target_screen, masker_screen):
        ...
        crosshair = visual.ShapeStim(win1, vertices=((-0.2, 0), (0.2, 0), (0,0), (0,-0.1), (0, 0.1)), lineWidth=20, closeShape=False, lineColor='white')
        crosshair.draw()
        win1.flip()
    ...
    def run_top_bottom_control_trial(self, win, position, trial_type):
        # Depending on the trial_type, position the crosshair at the top or bottom
        if trial_type == 'top':
            pos = (0, 0.65)  # Adjust these values as needed
        elif trial_type == 'bottom':
            pos = (0, -0.65)  # Adjust these values as needed

        #self.send_neurospec_trigger()

        # Initialize crosshair at the given position
        crosshair = visual.ShapeStim(win, vertices=((-0.2, 0), (0.2, 0), (0,0), (0,-0.1), (0, 0.1)), lineWidth=20, closeShape=False, lineColor='white', pos=pos)
        crosshair.draw()
        win.flip()
        core.wait(3)  # Wait for 3 seconds

        # Clear buffer and flip like the other trials
        win.clearBuffer()
        win.flip()
        ...
        self.record_control_trial_data(position)

    def run_eye_blink_control_trial(self):
        # Depending on the trial_type, position the crosshair at the top or bottom


        announcement = visual.TextStim(self.win3, text=f"BLINK", pos=(0, 0),height=0.05)
        announcement.draw() 
        self.win3.flip()
        core.wait(1)
        self.win3.flip()
        ...
        announcement = visual.TextStim(self.win3, text=f"STOP", pos=(0, 0),height=0.05)
        announcement.draw() 
        self.win3.flip()
        core.wait(1)
        self.win3.flip()

        inter_trial_interval = random.uniform(13, 15)
        ...
        # Record the trial data
        self.record_control_trial_data('blink')
    ...
    def run_control_block(self):
        ...
        top_bottom_selections = ['top'] * 5 + ['bottom'] * 5
        random.shuffle(top_bottom_selections)
        ...
        announcement = visual.TextStim(self.win3, text=f"Starting top down control block. Press spacebar to continue", pos=(0, 0),height=0.05)
        announcement.draw() 
        self.win3.flip()    
        event.waitKeys(keyList=["space"])

        for i in range(10):
            trial_type = top_bottom_selections.pop()
            # Assuming self.win3 is the window where the crosshair should be displayed
            self.run_top_bottom_control_trial(self.win3, trial_type, trial_type)


        announcement = visual.TextStim(self.win3, text=f"Starting blinking block. Press spacebar to continue", pos=(0, 0),height=0.05)
        announcement.draw() 
        self.win3.flip()    
        event.waitKeys(keyList=["space"])

        for i in range(5):
            self.run_eye_blink_control_trial() 
    ...
    def run_full_experiment(self):
        ...
        for block_method_name in self.block_methods:
            ...
            for target_movie, masker_movie in next_block_movie_pairs:
                ...
                continue_experiment = block_method(windows[target_screen], windows[masker_screen], windows[center_screen], target_movie, masker_movie, target_screen, masker_screen, center_screen)
